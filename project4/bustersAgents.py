...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    positions = []
    for curDistribution in livingGhostPositionDistributions:
        positions.append(curDistribution.argMax())
    
    actionDistance = util.Counter()
    for action in legal:
        successorPosition = Actions.getSuccessor(pacmanPosition, action)
        distances = [self.distancer.getDistance(successorPosition, pos2) for pos2 in positions]
        actionDistance[action] = -min(distances)
