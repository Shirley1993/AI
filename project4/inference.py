...
class ExactInference(InferenceModule):
  ...
  updates  compute the exact belief function at each time step.
  """
  ...
  def elapseTime(self, gameState):
    ...
    for pos in self.legalPositions:
        ...
        for newPos, newProb in newPosDist.items():
            tempbeliefs[newPos] += self.beliefs[pos] * newProb
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    #self.weights = [1.0] * self.numParticles 
...
#         self.weights[index] = self.weights[index] * emissionModel[trueDistance]
    
    if noisyDistance == None:
        self.particles = [self.getJailPosition()] * self.numParticles
    else: #resample
        #curBelief = self.getBeliefDistribution()
        curBelief = util.Counter()
        #probDistribution = util.Counter()
        for particle in self.particles:
            trueDistance = util.manhattanDistance(particle, pacmanPosition)
            #probDistribution[particle] += emissionModel
            curBelief[particle] += emissionModel[trueDistance]
            
        if (curBelief.totalCount() == 0):
            self.initializeUniformly(gameState)
        else:
            newParticles = []
            for particle in self.particles:
                newParticles.append(util.sampleFromCounter(curBelief))
  ...
  def elapseTime(self, gameState):
    ...
    for particle in self.particles:
        ...
        tmpParticles.append(util.sampleFromCounter(newPosDist))
...
#     for index, particle in enumerate(self.particles):
#         curBeliefs[particle] += self.weights[index]
    
    for particle in self.particles:
        curBeliefs[particle] += 1
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles):
        ...
        for j in range(self.numGhosts):
            newPositions.append(random.choice(self.legalPositions))
        self.particles.append(tuple(newPositions))
    ...
    for oldParticle in self.particles:
      ...
      for i in range(self.numGhosts):
          newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                   i, self.ghostAgents[i])
          tmpParticle.append(util.sampleFromCounter(newPosDist))
      newParticles.append(tuple(tmpParticle))
  ...
  def observeState(self, gameState):
    ...
    for particle in self.particles:
        ...
        updatedParticles.append(tuple(newParticle))
    self.particles = updatedParticles
    
    probDistribution = util.Counter()
    for index, particle in enumerate(self.particles):
        curParticle = list(particle)
        weight = 1
        for i in range(self.numGhosts):
            if noisyDistances[i] != None:
                trueDistance = util.manhattanDistance(curParticle[i], pacmanPosition)
                #self.weights[index] *= emissionModels[i][trueDistance]
                weight *= emissionModels[i][trueDistance]
        #probDistribution[particle] += self.weights[index]
        probDistribution[particle] += weight
    
    #if sum(self.weights) == 0:
    if (probDistribution.totalCount() == 0):
        self.initializeParticles()
        updatedParticles = []
        for particle in self.particles:
            newParticle = list(particle)
        ...
        newParticles = []
        for particle in self.particles:
            newParticle = util.sampleFromCounter(probDistribution)
            #print (newParticle)
